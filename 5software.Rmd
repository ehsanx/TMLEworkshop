...
```{r}
saveRDS(sl_fit, file = "data/sl3.RData")
```


```{r sl305b, cache=cachexy, echo=FALSE}
print(sl3_est)
...
```{r}
saveRDS(tmle.fit, file = "data/tmle.RData")
```
...
```{r}
saveRDS(ltmle_est, file = "data/ltmle.RData")
```
...
```{r aipwpkg, cache=cachex, results='hide', message=FALSE, warning=FALSE}
set.seed(1444) 
...
```{r aipwpkg2, cache=cachex, results='hide', message=FALSE, warning=FALSE}
# fit AIPW object
aipw$fit()
```


```{r aipwpkg3, cache=cachex, results='hide', message=FALSE, warning=FALSE}
# calculate ATE
aipw$summary()
...
```{r}
saveRDS(aipw, file = "data/aipw.RData")
```
